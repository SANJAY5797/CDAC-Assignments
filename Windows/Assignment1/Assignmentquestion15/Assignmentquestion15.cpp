...
int main()
{
	...
	BOOL bTerm = TerminateThread(hThread, dwExitThread);
	if (bTerm == 0)
	{
		_tprintf(TEXT("TerminateThread failed with error %d\n"), GetLastError());
		getchar();
		return 0;
	}
	GetExitCodeThread(hThread, &dwExitThread);
	_tprintf(TEXT("secondary thread exited with code %d\n"), dwExitThread);
	CloseHandle(hThread);
